...
const main = () => {
  ...
  // ----- Remove modal content to DOM ----- (enabled action out of call_modal ajax method when user goes back)
  $(".close-modal").click(function(){
    $('.modal-container-ajaxified').removeClass('active');
    setTimeout(function(){$('.modal-container-ajaxified').parent().remove();}, 200);
  });
...
// restore selectize elements before turbolinks cache
document.addEventListener('turbolinks:before-cache', function(){
  var selects = $('select.selectize');

  if(selects.length){
    for(var i = 0; i < selects.length; i++){
      $(selects[i]).selectize()[0].selectize.destroy();
    };
  };
});
